...
```
1일차: 상점 방문 → 점주가 "아, 김철수님이네요!" (신원 확인)
      → 점주가 고객 카드에 정보 기록 📝
      → 손님에게 "회원번호 1234번" 스티커 전달

2일차: 스티커만 보여주면 → 점주가 "아! 1234번 김철수님!"
      → 카드 보고 "어제 사탕 사셨던 분이죠?" (이전 상태 기억)
```
...
2일차: 다시 신분증 보여줘야 함 → 점주가 매번 새로 확인
      → 점주는 어제 뭘 샀는지 기억 안함 (상태 저장 안함)
```
...
```
로그인 → 서버: "철수가 로그인했구나, 기억해두자!"
        서버 메모리: { session123: "철수 정보" }
        브라우저: 쿠키에 "session123" 저장

다른 페이지 접속 → 브라우저: "session123이에요"
                서버: "아! session123은 철수구나!" (기억에서 찾음)
```

### JWT 방식

```
로그인 → 서버: "철수 맞네, 신분증(JWT) 발급해줄게"
        브라우저: JWT 토큰 저장

다른 페이지 접속 → 브라우저: JWT 토큰 제출
                서버: JWT 확인 → "이 토큰이 유효하니까 철수구나"
                (서버는 철수를 따로 기억하고 있지 않음)
```

## 핵심 차이점

**세션**: "너 누군지 내가 기억하고 있어" 🧠  
**JWT**: "너가 매번 신분증 보여줘, 그럼 확인해줄게" 🆔

- 세션이 있다 = 로그인된 상태로 서버가 인식
- 세션이 없다/만료 = 로그아웃된 상태
